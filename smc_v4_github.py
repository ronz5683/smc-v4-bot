...
LIMIT = 100
MAX_POSITIONS = 3
MAX_DAILY_SL = 3  # V8: stop after 3 SL  # untuk $20, max 3 posisi bareng biar tidak overtrade
...
DEFAULT_MAX_LEV = 50
BUFFER_PCT = 0.002  # V8: 0.2% buffer  # 0.15% buffer anti liquidity hunter
MIN_SL_PCT = 0.005  # V8: 0.5% min anti 9x loss  # min SL 0.15% biar tidak kesweep wick
...
def is_duplicate_entry(valid_new, closed_positions, symbol, entry):
    import datetime
    from datetime import timezone
    now = datetime.datetime.now(timezone.utc)
    for c in closed_positions[-20:]:
        if c['symbol']==symbol:
            try:
                ts = datetime.datetime.fromisoformat(c.get('closed_at','').replace('Z','+00:00'))
                if (now - ts).total_seconds() < 7200 and abs(c['entry']-entry)/entry < 0.001:
                    return True
            except:
                pass
    return False
...
def analyze_symbol_multi(symbol):
    ...
    # V8: HTF must align - kalau bearish jangan ambil bullish sweep
    if choch.get('type')=='BULLISH_CHOCH' and htf['trend']=='BEARISH':
        total_score -= 2  # penalty counter-trend
