...
def detect_htf_trend(k1h):
    ...
    last=k1h[-1]['close']
    if last>ema20>ema50: return {"trend":"BULLISH","score":1}
    if last<ema20<ema50: return {"trend":"BEARISH","score":1}
...
def detect_volume(candles):
    ...
    if last>avg*1.5: return {"score":1}
    if last>avg*1.2: return {"score":0.5}
    return {"score":0}
...
def check_active():
    ...
    for pos in active_positions:
        ...
        if not kl: new.append(pos); continue
            ...
            if low<=pos["sl"]:
                ...
                if (datetime.datetime.now(timezone.utc)-created).total_seconds()>86400 and pos.get("status")=="WAITING_LIMIT":
                    ...
                    if pos.get("status")=="WAITING_LIMIT":
                        entry=pos["entry"]
                        if any(c['low']<=entry<=c['high'] for c in kl[-96:]): pos["status"]="FILLED"
                    new.append(pos)
        else:
            if high>=pos["sl"]:
                pos["close_status"]="SL_HIT"; pos["pnl_rrr"]=-1; pos["closed_at"]=datetime.datetime.now(timezone.utc).isoformat(); closed_positions.append(pos); positions_data["stats"]["losses"]+=1; positions_data["stats"]["total_pnl"]-=1
...
for sym in SYMBOLS:
    if sym in active_symbols and len(active_positions)>=MAX_POSITIONS:
        results.append({"symbol":sym,"status":"SKIP","filter":"max_pos","confluence_score":0}); continue
    ...
    price=k15[-1]['close']; sweep=detect_sweep(k15); choch=detect_choch(k15); zones=detect_zones(k15); htf=detect_htf_trend(k1h); vol=detect_volume(k15)
...
def build_paper():
    ...
    for pos in sorted(closed_positions, key=lambda x: x.get('closed_at','')):
        r=pos.get('pnl_rrr',0); pnl = PAPER_RISK_USD*r - (pos.get('sizing',{}).get('notional_usd',200)*0.001 if r!=0 else 0)
        capital+=pnl; capital=max(capital,0.1); curve.append(round(capital,2))
        history.append({"symbol":pos.get('symbol'),"direction":pos.get('direction'),"entry":pos.get('entry'),"sl":pos.get('sl'),"tp":pos.get('tp'),"close_status":pos.get('close_status'),"pnl_r":r,"pnl_usd":round(pnl,4),"capital_after":round(capital,4),"closed_at":pos.get('closed_at','')[:19]})
    wins=len([h for h in history if h['pnl_r']>0]); losses=len([h for h in history if h['pnl_r']<0]); total_r=sum(h['pnl_r'] for h in history)
    data={"generated_at":datetime.datetime.now(timezone.utc).isoformat(),"summary":{"initial":PAPER_START_CAPITAL,"final":round(capital,2),"pnl_usd":round(capital-PAPER_START_CAPITAL,2),"total_trades":len(history),"wins":wins,"losses":losses,"win_rate":round(wins/len(history)*100,1) if history else 0,"total_r":round(total_r,2)},"trades":history,"active_positions":active_positions}
    with open(PAPER_FILE,'w') as f: json.dump(data,f,indent=2)

build_paper()
out={"last_scan_utc":datetime.datetime.now(timezone.utc).isoformat(),"summary":{"valid_new":len(valid_new),"active_positions":len(active_positions)},"results":results,"valid_new_positions":valid_new,"running_positions":active_positions}
with open("last_scan.json","w") as f: json.dump(out,f,indent=2)
print(f"DONE V6 FINAL: {len(valid_new)} NEW")
